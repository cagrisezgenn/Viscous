...
MAX_GEN_QUICK = 2;    % Çok az jenerasyon (Belki 15-20 de olabilir)
...
%   f1 ve f2 fiziksel birimlerini korurken f_pen boyutsuzdur.
    ...
    W.T    = util_getfield_default(W,'T',0.5);
    W.mu   = util_getfield_default(W,'mu',0.5);
    cav_free = util_getfield_default(penopt,'cav_free',0.02);  % [-] ≈ %0.5 serbest bölge
...
% Boyut analizi: Enerji alanları [J], güç = Δp·q [W], pen boyutsuzdur.
    ...
    cav_free = util_getfield_default(penopt,'cav_free',0.5);
...
cfg.num = struct('softmin_eps', 1e6, 'mu_min_phys', 0.6, 'dP_cap', 1e8, 'Qcap_scale', 3.2);
...
params = util_recompute_damper_params(params);
    ...
    if isfield(params,'orf') && isfield(params.orf,'CdInf') && ...
        ...
        dP_cap = util_getfield_default( ...
                     ...
                     'num', struct());
        dP_cap = util_getfield_default(dP_cap,'dP_cap', 1e8);  % [Pa], varsayılan 40 MPa
...
% alanları ile override yapabilir; eksik alanlar varsayılanla doldurulur.
    ...
    mu_phys_loc = util_getfield_default(num_loc,'mu_min_phys',0.6);
