...
[x,a,diagOut] = mck_with_damper(t,ag,M,C0,K, k_sd,c_lam0, use_orifice, orf, rho, Ap, A_o, Qcap_big, mu_ref, ...
...
nStories = size(diagOut.drift,2);
q = @(x,p) quantile(x,p);

rows = (1:nStories).';
ratio = diagOut.E_orifice / max(diagOut.E_struct, eps);
T = table(rows, ...
    q(abs(diagOut.drift),0.95).', ...
    q(abs(diagOut.story_force),0.95).', ...
    q(diagOut.Q,0.50).', q(diagOut.Q,0.95).', ...
    q(diagOut.dP_orf,0.50).', q(diagOut.dP_orf,0.95).', ...
    100*mean(diagOut.dP_orf<0).', ...
    q(abs(diagOut.PF),0.95).', ...
    repmat(diagOut.T_oil(end),nStories,1), ...
    repmat(diagOut.T_steel(end),nStories,1), ...
    repmat(diagOut.mu(end),nStories,1), ...
    repmat(diagOut.energy(end),nStories,1), ...
    repmat(diagOut.E_orifice,nStories,1), ...
    repmat(diagOut.E_struct,nStories,1), ...
    repmat(ratio,nStories,1), ...
    repmat(diagOut.P_mech,nStories,1), ...
...
for i=1:nStories
    ...
    c_eq = diagOut.c_lam * (Rvec(i)^2) * multi(i);
    kM = k_eq * [1 -1; -1 1];
    cM = c_eq * [1 -1; -1 1];
...
function [x,a,diagOut] = mck_with_damper(t,ag,M,C,K, k_sd,c_lam0, use_orf,orf,rho,Ap,Ao,Qcap, mu_ref, ...
    use_thermal, thermal, T0_C,T_ref_C,b_mu, c_lam_min,c_lam_cap,Lgap, ...
    cp_oil,cp_steel, steel_to_oil_mass_ratio, toggle_gain, story_mask, ...
    n_dampers_per_story, resFactor, cfg)
    ...
    diagOut = struct('drift',drift,'dvel',dvel,'story_force',F_story,'Q',Q, ...
