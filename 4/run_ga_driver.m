...
% Gelen girdileri ayrıştır, eksikse temel çalışma alanından tamamla.
    ...
    if ischar(scaledOrSnap_local) || isstring(scaledOrSnap_local)
        ...
        meta.mu_factors = Utils.getfield_default(S,'mu_factors',[0.70 1.00 1.30]);
        meta.mu_weights = Utils.getfield_default(S,'mu_weights',[0.3 0.5 0.2]);
        ...
        meta = struct('IM_mode','', 'band_fac',[], 's_bounds',[], ...
                      'mu_factors',[0.70 1.00 1.30], 'mu_weights',[0.3 0.5 0.2], ...
    ...
    % Ceza/penalty varsayilanlari (kullanici verebilir)
    if ~isfield(optsEval,'penalty_scale'),   optsEval.penalty_scale = 5; end
    if ~isfield(optsEval,'penalty_power'),   optsEval.penalty_power = 1.0; end
    if ~isfield(optsEval,'penalty_weights')
    ...
    options = optimoptions('gamultiobj', ...
       ...
       'CrossoverFraction', Utils.getfield_default(optsGA,'CrossoverFraction',0.8), ...
       'MutationFcn',       Utils.getfield_default(optsGA,'MutationFcn',{@mutationgaussian,0.15,0.4}), ...
       'ParetoFraction',    Utils.getfield_default(optsGA,'ParetoFraction',0.50), ...
    ...
    try
        if isfield(optsGA,'InitialPopulationMatrix') && ~isempty(optsGA.InitialPopulationMatrix)
            options = optimoptions(options,'InitialPopulationMatrix', optsGA.InitialPopulationMatrix);
        else
    ...
    if K > 0
        ...
        % --- Local refinement with patternsearch (continuous vars only) ---
        try
            do_refine = exist('patternsearch','file')==2;
        catch
            do_refine = false;
        end
        if do_refine
            try
                cont_idx = setdiff(1:numel(lb), IntCon);
                f1v = F(:,1); f2v = F(:,2);
                f1min = min(f1v); f1max = max(f1v);
                f2min = min(f2v); f2max = max(f2v);
                normf = @(f) [ (f(1)-f1min)/max(eps,(f1max-f1min)), (f(2)-f2min)/max(eps,(f2max-f2min)) ];
                optsPS = optimoptions('patternsearch','Display','off','UseCompletePoll',true, ...
                    'UseCompleteSearch',false,'MaxIterations',50,'MeshTolerance',1e-3);

                top_refined = table();
                for i = 1:numel(idx)
                    x0 = X(idx(i),:);
                    xi0 = x0(cont_idx);
                    lbc = lb(cont_idx); ubc = ub(cont_idx);
                    obj_ps = @(xc) local_scalar_obj(xc, x0, cont_idx, scaled, params, optsEval, normf);
                    [xc_best, ~] = patternsearch(obj_ps, xi0, [], [], [], [], lbc, ubc, [], optsPS);
                    x_best = x0; x_best(cont_idx) = xc_best; x_best = quant_clamp_x(x_best);
                    [f_best, ~] = eval_design_fast(x_best, scaled, params, optsEval);
                    Pbest = decode_params_from_x(params, x_best);
                    row = table(Pbest.orf.d_o*1e3, Pbest.n_orf, ...
                        x_best(3), x_best(4), x_best(5), x_best(6), x_best(7), x_best(8), ...
                        x_best(9), x_best(10), x_best(11), x_best(12), x_best(13), x_best(14), ...
                        Pbest.A_o, Pbest.Qcap_big, f_best(1), f_best(2), 'VariableNames', ...
                        {'d_o_mm','n_orf','PF_tau','PF_gain','Cd0','CdInf','p_exp','Lori_mm','hA_W_perK','Dp_mm','d_w_mm','D_m_mm','n_turn','mu_ref','A_o','Qcap_big','f1','f2'});
                    top_refined = [top_refined; row]; %#ok<AGROW>
                end
                safe_write(top_refined, fullfile(outdir,'ga_topK_refined.csv'), @writetable);
            catch ME
                warning('patternsearch refine failed: %s', ME.message);
            end
        end
    end
...
function [f, meta] = eval_design_fast(x, scaled, params_base, optsEval)
    ...
    % Build a richer memo key so changes in dataset/penalty/mu/thr invalidate cache
    try
        Otmp = struct();
        if nargin >= 4 && ~isempty(optsEval), Otmp = optsEval; end
        if ~isfield(Otmp,'mu_factors'), Otmp.mu_factors = [0.70 1.00 1.30]; end
        if ~isfield(Otmp,'mu_weights'), Otmp.mu_weights = [0.3 0.5 0.2]; end
        if ~isfield(Otmp,'thr'),        Otmp.thr = Utils.default_qc_thresholds(struct()); end
        Wtmp = Utils.getfield_default(Otmp,'penalty_weights', struct('dP',1,'Qcap',1,'cav',3,'T',1,'mu',0.5));
        compKey = struct('x',x,'ds',dsig,'mf',Otmp.mu_factors,'mw',Otmp.mu_weights, ...
                         'thr',[Otmp.thr.dP95_max, Otmp.thr.Qcap95_max, Otmp.thr.cav_pct_max, Otmp.thr.T_end_max, Otmp.thr.mu_end_min], ...
                         'ps', Utils.getfield_default(Otmp,'penalty_scale',5), ...
                         'pp', Utils.getfield_default(Otmp,'penalty_power',1.0), ...
                         'W',[Wtmp.dP, Wtmp.Qcap, Wtmp.cav, Wtmp.T, Wtmp.mu]);
        key = jsonencode(compKey);
    catch
    ...
    if ~isfield(O,'mu_factors'), O.mu_factors = [0.70 1.00 1.30]; end
    if ~isfield(O,'mu_weights'), O.mu_weights = [0.3 0.5 0.2]; end
    ...
    hk = Utils.getfield_default(optsEval,'hardkill', struct('dP',1.5,'Qcap',1.2,'cav_abs',0.02));
    thrHK = O.thr; if ~isfield(thrHK,'dP95_max'), thrHK.dP95_max = inf; end
    if ~isfield(thrHK,'Qcap95_max'), thrHK.Qcap95_max = 1; end
    if ~isfield(thrHK,'cav_pct_max'), thrHK.cav_pct_max = 0; end
    dP_lim   = hk.dP   * thrHK.dP95_max;
    qcap_lim = hk.Qcap * thrHK.Qcap95_max; if ~isfinite(qcap_lim) || qcap_lim<=0, qcap_lim = 0.95; end
    if thrHK.cav_pct_max>0
        cav_lim = max(10*thrHK.cav_pct_max, 1e-3);
    else
        cav_lim = hk.cav_abs;
    end
    if any(dP95v > dP_lim) || any(qcapv > min(0.95,qcap_lim)) || any(cavv > cav_lim)
        f = [1e6, 1e6];
    ...
    rel = @(v,lim) max(0, (v - lim)./max(lim,eps)).^pwr;
    rev = @(v,lim) max(0, (lim - v)./max(lim,eps)).^pwr;
    W = Utils.getfield_default(optsEval,'penalty_weights', struct('dP',1,'Qcap',1,'cav',3,'T',1,'mu',0.5));
...
function val = local_scalar_obj(xc, x_fixed, cont_idx, scaled, params, optsEval, normf)
    x = x_fixed; x(cont_idx) = xc; x = quant_clamp_x(x);
    [f, ~] = eval_design_fast(x, scaled, params, optsEval);
    fn = normf(f);
    val = fn(1)^2 + fn(2)^2;
end
