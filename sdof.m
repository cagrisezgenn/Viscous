...
function [x,a_rel,ts] = mck_with_damper_local(t,ag,M,C,K, k_sd,c_lam0,Lori, orf,rho,Ap,Ao,Qcap, mu_ref, ...
    ...
    function dz = damper_rhs_local(tt,z,ctx)
        ...
        p_pf_loc = orf_loc.dP_eff .* sign(dvel + 0);
        if ctx.pf_res_only
            s_loc = tanh(ctx.gate_k*dvel);
            p_pf_loc = s_loc .* max(0, s_loc .* p_pf_loc);
        end
        F_story_loc = F_lin_loc + ctx.Ap_story_col .* p_pf_loc;
        F_loc = zeros(ctx.n,1);
        for ii = 1:ctx.Ns
    ...
    function out = compute_orifice_terms_local(dvel, F_lin, mu_val, rho_val, ctx)
        ...
        Re = (rho_mat .* Q_abs) .* ctx.d_o_single ./ max(Ao_mat .* mu_mat, 1e-9);
        ...
        dP_kv = 0.5 * rho_mat .* (Q_abs ./ max(Cd .* Ao_mat, 1e-12)).^2;
