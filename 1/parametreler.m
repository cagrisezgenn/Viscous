...
m  = 360e3 * ones(n,1);
k  = 6.5e8 * ones(n,1);
c  = 6.2e6 * ones(n,1);      % (artık sadece referans; C0'ı Rayleigh ile kuracağız)
story_height = 3.0;

% Kütle, rijitlik ve "geçici" C0 matrisi (C0'ı az sonra Rayleigh ile değiştireceğiz)
M  = diag(m);
...
for i = 1:n
    ...
    K(i,i)   = kL + kU;
    % C0 burada kurulu ama birazdan Rayleigh ile ÜZERİNE YAZILACAK
    C0(i,i)  = cL + cU;
    if i > 1
        K(i,i-1)  = -kL;
        C0(i,i-1) = -cL;
...
[VV,DD] = eig(K,M);
ww  = sqrt(sort(diag(DD),'ascend'));
w1  = ww(1);               % rad/s
T1  = 2*pi/w1;

% ================= Rayleigh sönümünü burada kur =================
% hedef: ζ ≈1% @ ω1, ≈4–5% @ 4·ω1
whf = 4*w1;  z1 = 0.01;  zhf = 0.05;
A = [1/(2*w1)  ,  w1/2;
     1/(2*whf) ,  whf/2];
ab    = A \ [z1; zhf];
alpha = ab(1);
...
% Yapısal sönüm matrisi: Rayleigh
C0 = alpha*M + beta*K;
...
n_orf = 4;         % Kat başına orifis sayısı
...
toggle_gain = [3.2 3.2 3.2 3.4 3.4 3.4 3.4 3.6 3.6].';  % Toggle kazancı (skaler veya (n-1)x1 vektör)
n_dampers_per_story = 1*ones(n-1,1);                    % Kat başına damper adedi (skaler veya (n-1)x1 vektör)
...
resFactor = 5;                                            % Hacim/kapasite ölçeği
